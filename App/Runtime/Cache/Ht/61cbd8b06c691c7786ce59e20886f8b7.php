...
<script type="text/javascript" src="/minimeidea_duo/Public/ht/js/mydate.js"></script>
<style>
        ...
        <li class="product"><div class="d1 dx2">优惠券信息</div></li>
         ...
         <li>
            ...
            <div>
              ...
              <span style="color:red;margin-left:5px;">&nbsp;* 优惠券发放数量</span>
            </div>
         </li>
         <li>
            <div class="d1">使用范围:</div>
            <div>
              <input class="inp_1" id="cate_id" name="cate_id" value="<?php echo $voucher['cate_id']==''?'0':$voucher['cate_id']; ?>" readonly="readonly"/>
              <input type="button" value="选择分类" class="aaa_pts_web_3" style="margin-left:15px;" onclick="win_open('<?php echo U('get_pro');?>',1280,800)">
              <span style="color:red;margin-left:5px;">&nbsp;* 默认店铺内所有商品分类通用</span>
            </div>
         </li>
         <?php if ($voucher['cate_id']!='all' && $voucher['cate_id']!='' && $voucher['pro_list']) { ?>
          <li>
            <div class="d1">已选产品:</div>
            <div>
              <?php if(is_array($voucher['pro_list'])): $i = 0; $__LIST__ = $voucher['pro_list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><img src="/minimeidea_duo/Data/<?php echo ($v); ?>" width="100px" height="100px" /><?php endforeach; endif; else: echo "" ;endif; ?>
            </div>
          </li>
         <?php } ?>
